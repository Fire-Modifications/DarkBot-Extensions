    ...
    Gamedig.query({
      ...
      .then(async (state) => {
        ...
        const embed = new MessageEmbed()
          ...
          .addField(
            "**Online Players:**",
            `**Total:** \`${state.raw.clients}\` / \`${state.raw.sv_maxclients}\``,
            true
          )
          .addField(
            "**Current Players:**",
            `${players.join(",  ").toString()}`
          );
        const maindiscord = client.guilds.cache.find(
          (g) => g.id === conf.guildId
        );
        const statuschannel = maindiscord.channels.cache.find(
          (c) => c.id === conf.statusChannel
        );
